...
function paginator($format_string, $current_page, $num_pages) {
  ...
  if(isset($_GET['orderby'])) {
    $get_variables['orderby'] = $_GET['orderby'];
  }
  if(isset($_GET['sortdir'])) {
    $get_variables['sortdir'] = $_GET['sortdir'];
  }
...
function tableHeader($url, $columns) {
  ...
  $get_variables = array();
  if(isset($_GET['page'])) {
    $get_variables['page'] = intval($_GET['page']);
  } else {
  ...
  if(isset($_GET['orderby'])) {
    ...
    $get_variables['orderby'] = 'id';
  }
  if(isset($_GET['sortdir']) && $_GET['sortdir'] == 'desc') {
    $get_variables['sortdir'] = 'asc';
  } else {
    $get_variables['sortdir'] = 'desc';
  }
  foreach($columns as $key => $val) { 
    ...
    if(isset($_GET['sortdir'])) {
      if(isset($_GET['orderby']) && $_GET['orderby'] == $key) {
        if($get_variables['sortdir'] == 'asc') {
          $fa = '<i class="fa fa-arrow-down"></i> ';
        } else {
